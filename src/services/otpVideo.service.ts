...
async function processWatermarkingInBackground(
  ...
  try {
    // S3 storage keys
    const watermarkedStorageKey = `${sessionId}/watermarked/otpVideo.webm`;
    ...
    console.log('OTP video watermarked successfully');
    ...
    if (supabaseS3) {
      try {
        ...
        console.error('Error uploading watermarked OTP video to S3:', watermarkedUploadError);
